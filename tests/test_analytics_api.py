    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success') and data.get('data'):
                ...
                print(f"   Top Department: {summary.get('topDepartment', 'N/A')}")
                ...
                for att in attendance_data[:3]:  # Show first 3 months
                    print(f"   {att['month']}: {att['present']} present, {att['absent']} absent ({att['attendance']}%)")
                
                # Determine if data is from MongoDB or fallback
                if students == "11" and len(dept_data) == 1 and dept_data[0]['name'] == 'CSE':
                    print("\n✅ REAL MONGODB DATA - Using actual database values!")
                    print("   (11 students, 4 faculty from College_db)")
                else:
                    print("\n⚠️  FALLBACK DATA - Using static values")
                    print("   (MongoDB connection may have issues in the request)")
